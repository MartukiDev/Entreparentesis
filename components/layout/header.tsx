          ...
          {navigation.map((item) => (
            <Link
              ...
              className={`text-sm font-semibold leading-6 ${
                pathname === item.href ? "text-red-600 border-b-2 border-red-600" : "text-gray-900 hover:text-red-600"
              }`}
            >
              {item.name}
            </Link>
        ...
        <div className="lg:hidden">
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            <div className="flex items-center justify-between">
              <Link href="/" className="-m-1.5 p-1.5">
                <span className="sr-only">Arte Entreparéntesis</span>
                <h2 className="text-xl font-bold">Arte Entreparéntesis</h2>
              </Link>
              <button
                type="button"
                  ...
                  {navigation.map((item) => (
                    <Link
                      ...
                      className={`-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${
                        pathname === item.href ? "text-red-600 bg-gray-50" : "text-gray-900 hover:bg-gray-50"
                      }`}
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {item.name}
